...
import {
  parseUserGuidString,
  pickClubEmployeePersonLookupKey,
  pickClubRosterLookupKey,
  pickMemberUserGuidFromApiDto,
  pickUserGuidForAuthLookup
} from '../../utils/userGuids'
...
const ClubAdminMembers = () => {
  ...
  const loadPeople = useCallback(async () => {
    ...
    try {
      const [clubRes, memRes, empRes, posRes] = await Promise.all([
        fetchClub(clubId),
        fetchClubAdminMembers(clubId),
        fetchClubAdminEmployees(clubId).catch(() => ({ items: [] })),
      ...
      const uniqueMemberKeys = Array.from(
        new Set(baseMembers.map((m) => m.userId).filter(Boolean))
      )
      ...
      const employeeSource = rawOfficers.length ? rawOfficers : empItems

      const baseEmployees = (Array.isArray(employeeSource) ? employeeSource : []).map((e, index) => {
        ...
        const lookupKey =
          pickUserGuidForAuthLookup(e) ||
          pickClubEmployeePersonLookupKey(e) ||
          ''

        const pre = personNamePartsFromClubRosterDto(e)

        return {
          id: String(e.id ?? e.employeeId ?? e.officerId ?? `e-${index}`),
          userId: lookupKey,
          name: pre.name,
          surname: pre.surname,
          email: pre.email,
      ...
      const uniqueEmployeeKeys = Array.from(
        new Set(baseEmployees.map((e) => e.userId).filter(Boolean))
      )
          ...
          if (!p) {
            ...
            const fromNested =
              member && typeof member === 'object'
                ? displayNameFromAuthUserDto(member)
                : ''
            ...
            return {
              ...
              email:
                String(raw?.email ?? member?.email ?? member?.Email ?? '').trim() ||
                e.email,
            }
          }
  ...
  const handleUpdateEmployeePosition = (id, newPositionId) => {
    ...
    setEmployees((prev) =>
      prev.map((e) =>
        e.id === id
          ? { ...e, positionId: newPositionId, positionName: name }
          : e
      )
    )
  ...
  return (
    <div className="club-admin-page">
      ...
      <div className="club-admin-content">
        {loadError ? (
          <p style={{ margin: '0 24px 20px', fontSize: 14, color: '#b91c1c' }}>
            {loadError}
          </p>
        ) : null}
        ...
        <div className="club-admin-card" style={{ margin: '0 24px 24px' }}>
          <div
            className="club-admin-sidebar-nav"
            style={{
              flexDirection: 'row',
              padding: 0,
          ...
          >
            ...
            >
              Members
              <span className="club-admin-pill club-admin-pill--pending" style={{ marginLeft: 8 }}>
                {members.length}
              </span>
            </button>
            ...
            {activeTab === 'members' ? (
              <table className="club-admin-table">
                ...
                <tbody>
                  {loading ? (
                    <tr>
                      <td colSpan={2} className="club-admin-table-empty">
                        Loading members...
                      </td>
                    </tr>
                  ) : null}
                    ...
                    <tr key={m.id}>
                      <td>
                        <div className="club-admin-table-user">
                          ...
                          <div>
                            <div className="club-admin-table-name">
                              {m.name} {m.surname}
                            </div>
                            <a href={`mailto:${m.email}`} className="club-admin-table-email">
                              {m.email}
                            </a>
                          </div>
                        </div>
                      ...
                      <td>
                        <div className="club-admin-table-user">
                          ...
                          <div>
                            ...
                            <a href={`mailto:${e.email}`} className="club-admin-table-email">
                              {e.email}
                            </a>
                          </div>
                        </div>
      ...
      {removeConfirm.id != null && (
        ...
        >
          <div
            className="club-admin-popup club-admin-popup--sm"
          ...
          >
            <div className="club-admin-popup-header">
              ...
              <button
                type="button"
                className="club-admin-popup-close"
                onClick={() => setRemoveConfirm({ id: null, type: null })}
                aria-label="Close"
              >
                ×
              </button>
