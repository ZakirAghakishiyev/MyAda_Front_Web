...
const STAFF_OPTIONS = Object.values(STAFF_BY_ID)
...
const AssignTask = () => {
  ...
  const staffIdFromQuery = queryParams.get('staffId')
  const initialStaffFromQuery = staffIdFromQuery ? STAFF_BY_ID[staffIdFromQuery] || '' : ''

  const [area, setArea] = useState(
    areaFromQuery === 'fm' ? 'fm' : 'it'
  )
  const [staff, setStaff] = useState(initialStaffFromQuery)
  const [issueCategory, setIssueCategory] = useState('')
  const [locationType, setLocationType] = useState('')
  const [building, setBuilding] = useState('')
  const [isRoomOrNonRoom, setIsRoomOrNonRoom] = useState('')
  ...
  const handleSubmit = (e) => {
    ...
    if (!staff || !issueCategory.trim() || !hasLocation || !description.trim()) {
      setShowError(true)
      return
    }
      ...
      <div className="it-support-popup">
        <header className="it-support-header">
          ...
          <div className="it-support-header-title">
            <h1 id="assign-task-title">Assign New Task</h1>
            <span className="it-support-subtitle">
          ...
          <form className="it-support-form it-support-form--wide" onSubmit={handleSubmit}>
            ...
            <section className="it-support-section">
              ...
              <div className="it-support-category-toggle">
                <button
                  type="button"
                  className={`it-support-category-pill ${
                    area === 'it' ? 'it-support-category-pill--active' : ''
                  }`}
                  onClick={() => {
                    setArea('it')
                    setIssueCategory('')
                ...
                <button
                  type="button"
                  className={`it-support-category-pill ${
                    area === 'fm' ? 'it-support-category-pill--active' : ''
                  }`}
                  onClick={() => {
                    setArea('fm')
                    setIssueCategory('')
            ...
            <section className="it-support-section">
              ...
              <div className="it-support-fields">
                <textarea
                  required
                  rows={5}
                  placeholder="Provide as much detail as possible about the task or issue..."
                  value={description}
                  onChange={(e) => setDescription(e.target.value)}
                  className="it-support-textarea"
                />
                <p className="it-support-hint">
                  Include any context the staff member needs, such as affected users, systems, or deadlines.
                </p>
              </div>
            </section>
            ...
            <section className="it-support-section">
              ...
              <div className="it-support-fields it-support-urgency-grid">
                <button
                  type="button"
                  className={`it-support-urgency-card ${
                    urgency === 'standard' ? 'it-support-urgency-card--active' : ''
                  }`}
                ...
                <button
                  type="button"
                  className={`it-support-urgency-card ${
                    urgency === 'critical' ? 'it-support-urgency-card--active' : ''
                  }`}
            ...
            {showError && (
              <p className="it-support-error" role="alert">
                Please fill in all required fields, including assigned staff.
              </p>
            )}
          ...
          <aside className="it-support-side-panel">
            <div className="it-support-info-box">
              ...
              <ul className="it-support-info-list">
                <li>A new task is created and assigned to the selected staff member.</li>
                <li>The task appears in the dispatcher tickets and staff workload views.</li>
                <li>The assignee can update status as they work on the task.</li>
              </ul>
            </div>
            ...
            <div className="it-support-info-box it-support-info-box--accent">
              ...
              <p className="it-support-info-text">
                For emergencies or campus‑wide outages, please also notify the duty manager directly.
              </p>
            </div>
          </aside>
        </div>
